...
export default function FollowingPage() {
  ...
  const fetchFollowing = async () => {
    setLoading(true);
    try {
      const res = await apiClient<any>(`/community/following?username=${username}`);
  ...
  const handleFollowToggle = async (userId: string) => {
    try {
      ...
      if (res.success) {
        const isFollowingNow = res.data.following;
        setFollowing(prev => prev.map(f => {
          if (f.id === userId) {
            return { ...f, isFollowing: isFollowingNow };
          }
          return f;
        }));
        triggerToast("Sync Successful", isFollowingNow ? "Signal connected" : "Signal separated", "success");
      }
    } catch (e) {}
  };
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-black text-white uppercase tracking-tight">Following Signals</h2>
