  ...
  const syncState = async () => {
    ...
    try {
      ...
      if (res.success && data && data.leaderboard) {
        ...
        const sorted: LeaderboardRankEntry[] = data.leaderboard.map((e: any) => ({
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4 bg-dash-card p-4 rounded-2xl border border-dash-border-subtle shadow-xl">
            <div className="text-right">
              ...
              <p className="text-dash-text font-black text-2xl">7 Days</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          >
            ...
            <h3 className="text-4xl font-black text-dash-text">{stat.value}</h3>
            <div className="w-full h-1.5 bg-dash-text/5 rounded-full overflow-hidden mt-4">
          ...
          <div className="relative z-10 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar mb-8 space-y-4 max-h-[220px]">
              {state.nextWorkout?.exercises.map((ex, i) => (
                <div key={i} className="bg-dash-text/5 border border-dash-border-subtle p-4 rounded-2xl group/ex hover:border-neon-blue/30 transition-all">
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-dash-text font-bold text-sm group-hover/ex:text-neon-blue transition-colors">{ex.name}</span>
